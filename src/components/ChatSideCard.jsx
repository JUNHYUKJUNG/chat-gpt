    ...
    <>
      ...
      {isOpen && (
        <div className="fixed top-0 left-0 bg-black bg-opacity-20 w-full h-full z-10">
          <div className="bg-white rounded-xl flex flex-col fixed p-4 shadow-lg top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
            <button onClick={onClickClose} className="self-end mb-4">
              <CgCloseR size={20} />
            </button>
            <div className="text-xl">Q. {question}</div>
            <div className="text-xl mt-5 text-semibold">A. {answer}</div>
          </div>
        </div>
      )}
    </>
